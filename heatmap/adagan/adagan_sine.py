...
flags.DEFINE_string("workdir", 'results_sine', "Working directory ['results']")
...
def main():
    ...
    opts['dataset'] = 'sine' # gmm, circle_gmm,  mnist, mnist3, cifar ...
...
if __name__ == '__main__':
    ...
    for _ in tqdm(range(1)):
        ...
        tik()
        main()
        tok("AdaGAN experiment")

        all_samples = []
        results = []
        for i in range(10):
            print("Iter {} / 10".format(i + 1))
            a = np.load('results_sine/samples{:02d}.npy'.format(i))
            a = a[:, :, 0, 0]
    ...
    plt.hist2d(tmp[:, 0], tmp[:, 1], bins=(100, 100), cmap=plt.cm.jet)
    plt.savefig("adagan_heatmap.png")
