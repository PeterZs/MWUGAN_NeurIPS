...
class DataHandler(object):
    ...
    def _load_data(self, opts):
        ...
        if opts['dataset'] == 'mnist':
            ...
            self._load_gmm(opts)
        elif opts['dataset'] == 'helix':
            self._load_helix(opts)
        elif opts['dataset'] == 'sine':
            self._load_sine(opts)
        elif opts['dataset'] == 'outlier':
            self._load_outlier(opts)
    ...
    def _load_gmm(self, opts):
        ...
        # First we choose parameters of gmm and thus seed
        modes_num = 10 # opts["gmm_modes_num"]
        np.random.seed(opts["random_seed"])
    ...
    def _load_helix(self, opts):
        logging.debug('Loading helix dataset...')
        ...
        # self.modes = np.array([[np.cos(i / 3.) * float(i) * float(i), np.sin(i / 3.) * float(i) * float(i)] for i in xrange(self.mode)])
        # self.modes = np.array([[np.cos(i) * float(i), np.sin(i) * float(i)] for i in xrange(self.mode)])
        data = None
        ...
        logging.debug('Loading Helix dataset done! {}'.format(data.shape))

    def _load_sine(self, opts):
        logging.debug('Loading sine dataset (y = x sinx)...')

        self.sig=0.5
        self.num_per_mode = 50
        self.isimg = False
        ...
        logging.debug('Loading Sine dataset done! {}'.format(data.shape))
    
    def _load_outlier(self, opts):
        logging.debug('Loading outlier dataset...')
        ...
        np.random.shuffle(data)
        # data = data[:, :, None, None]
        self.label = np.concatenate(labels, axis=0)[:, 0]
        ...
        logging.debug('Grid dataset loaded. Shape = {}'.format(self.data.shape))
